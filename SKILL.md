---
name: agentic-design
description: Design, write, and revise prompts, instructions, and architecture for AI agents. Use this skill whenever the user is creating or editing a system prompt for any agent - voice agents (inbound receptionist, appointment booking, estimate follow-up, outbound speed-to-lead, review/happy-call agents), text or chat agents (customer support, sales, internal assistants), or workflow automations with LLM steps - writing or updating a CLAUDE.md or agent instruction file, designing tools or function calls for an agent, deciding between single-agent and multi-agent architecture, debugging an agent that follows instructions poorly, or evaluating whether a prompt change is safe to ship. Trigger even when the user just says "write a system prompt," "fix the agent's prompt," "the agent keeps doing X," "write instructions for," or "should this be one agent or two."
---

# Agent Design

Rules for building agents that work. Grounded in replicated findings: over-specified prompts measurably degrade performance. Stacking requirements drops instruction-following by up to 19% (Yang et al. 2025, "What Prompts Don't Say", arXiv:2505.13360), rigid output formats tax reasoning by 10-30% (Tam et al. 2024, "Let Me Speak Freely?", arXiv:2408.02442), and roughly two-thirds of rules audited in real production prompts restated what the model already did by default. The job of an agent prompt is not to control every step. It is to give the model a clear goal, hard boundaries, and room to comprehend.
...
3. **Procedure altitude (rare):** Exact step-by-step scripts. Only for compliance-critical or genuinely deterministic sequences (payment capture, legal disclosures). If a procedure can live in code instead of the prompt, put it in code.
...
2. **For a new prompt, start from the matching skeleton in `assets/prompt-skeletons.md`.** Then write the goal block first: who the agent is, what outcome it must produce, what a successful interaction looks like. 3-6 sentences.
3. **Add hard constraints as a short flat list, each with its reason.** "Never quote below the cost floor because a job under it loses money" outperforms the bare rule: the model applies the reason to cases the rule did not anticipate. If the list passes ~10 items, question each one. Move anything enforceable in code (function gating, validation, routing) out of the prompt.
4. **Add 2-4 canonical examples** of ideal handling, including one hard case. Examples teach behavior more reliably than rules describing behavior.
5. **Cut ruthlessly.** For each remaining instruction ask: does this change behavior, or does it just make the author feel safe? Duplicated rules, hedges, and restated obvious behavior all cost attention. Shorter prompts are not lazier; they are higher-signal.
6. **Never ship a prompt change without the eval pass** in `references/evaluation.md`. A change that fixes one transcript can silently break five others.
